...
function strengthLabel(progress: TopicProgress | null): { text: string; color: string } {
  if (!progress || progress.total_attempts === 0) {
    return { text: 'Не начато', color: 'text-gray-400' }
  }
  ...
  if (loading) {
    return (
      <div className="p-8">
        ...
        <div className="mb-8 h-40 animate-pulse rounded-xl border border-gray-200 bg-gray-50" />
    ...
    <div className="p-8">
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm">
          ...
          {topic.estimated_study_hours && (
            <span className="text-gray-400">~{topic.estimated_study_hours} ч. на изучение</span>
          )}
      ...
      {topic.user_progress && topic.user_progress.total_attempts > 0 && (
        <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-4 text-center">
            ...
            <div className="text-xs text-gray-500">Сила темы</div>
          </div>
        </div>
      )}
      ...
      {topic.description && (
        <section className="mb-8">
          <h2 className="mb-3 text-lg font-semibold text-gray-800">Теория</h2>
      ...
      {relationships.length > 0 && (
        <section className="mb-8">
          <h2 className="mb-3 text-lg font-semibold text-gray-800">Связанные темы</h2>
            ...
            {relationships.map((rel) => (
              rel.related_topic && (
                <Link
                  ...
                  className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm transition-colors hover:border-blue-300 hover:bg-blue-50"
                >
                  ...
                  {rel.relationship_type === 'prerequisite' && (
                    <span className="ml-1.5 text-xs text-gray-400">(пререквизит)</span>
                  )}
                </Link>
              )
            ))}
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold text-gray-800">
          Задания <span className="text-sm font-normal text-gray-400">({problems.length})</span>
        </h2>
        {problems.length === 0 ? (
          <p className="text-sm text-gray-500">Задания по этой теме пока не добавлены.</p>
        ) : (
          <div className="space-y-3">
            {problems.map((problem, idx) => (
              <div
                key={problem.id}
                className="rounded-lg border border-gray-200 bg-white p-4 transition-colors hover:border-blue-200"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    <div className="mb-1 flex items-center gap-2">
                      <span className="text-sm font-medium text-gray-700">Задание {idx + 1}</span>
