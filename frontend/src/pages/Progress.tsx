...
function strengthBg(s: number): string {
  ...
  if (s > 0) return 'bg-orange-100'
  return 'bg-gray-50'
}

function actionColor(action: string): string {
  if (action.includes('FIRe')) return 'bg-red-100 text-red-700'
  if (action.includes('теорию')) return 'bg-amber-100 text-amber-700'
  return 'bg-blue-100 text-blue-700'
}

/** Build a 52-week × 7-day grid for the heatmap. */
...
export default function Progress() {
  ...
  if (loading) {
    return (
      <div className="p-8">
        <h1 className="mb-6 text-2xl font-bold text-gray-900">Прогресс</h1>
  ...
  if (error) {
    return (
      <div className="p-8">
        <h1 className="mb-4 text-2xl font-bold text-gray-900">Прогресс</h1>
        <p className="text-red-600">Ошибка загрузки: {error}</p>
      </div>
    )
  }

  /* ---- Filter gap map entries client-side ---- */
  ...
  return (
    <div className="p-8">
      ...
      <section className="mb-8">
        <h2 className="mb-4 text-lg font-semibold text-gray-900">Карта пробелов</h2>

        {/* Filters */}
        <div className="mb-4 flex flex-wrap items-end gap-4">
          <div>
            <label className="mb-1 block text-xs font-medium text-gray-500">Номер задания</label>
            <select
              value={filterTask}
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-gray-500">Сила от (%)</label>
            <input
              type="number"
              min={0}
              max={100}
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-gray-500">Сила до (%)</label>
            <input
              type="number"
              min={0}
              max={100}
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-gray-200 bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 font-medium text-gray-600">Сила</th>
                <th className="px-4 py-3 font-medium text-gray-600">Ошибки (30д)</th>
                <th className="px-4 py-3 font-medium text-gray-600">Посл. ошибка</th>
                <th className="px-4 py-3 font-medium text-gray-600">Рекомендация</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((e) => (
                  <tr key={e.task_number} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 font-medium text-gray-900">{e.topic}</td>
                    <td className="px-4 py-3">
                        ...
                        <div className="h-1.5 w-16 overflow-hidden rounded-full bg-gray-200">
                          <div
                            className={`h-full rounded-full transition-all ${
                              e.strength >= 70 ? 'bg-green-500' : e.strength >= 40 ? 'bg-yellow-500' : e.strength > 0 ? 'bg-orange-500' : 'bg-gray-300'
                            }`}
      ...
      <section className="mb-8">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900">Активность</h2>
          {calendar && (
        ...
        <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          ...
          <div className="mb-1 flex">
            ...
            {heatmapWeeks.map((week, wi) => {
              ...
              if (wi === 0 || curMonth !== prevMonth) {
                return (
                  <div key={wi} className="text-[10px] text-gray-400" style={{ width: 14, marginRight: 2 }}>
          ...
          {['Пн', '', 'Ср', '', 'Пт', '', ''].map((dayLabel, dayIdx) => (
            <div key={dayIdx} className="flex items-center">
              <div className="w-8 shrink-0 text-[10px] text-gray-400">{dayLabel}</div>
              {heatmapWeeks.map((week, wi) => {
      ...
      <section>
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900">Динамика решений</h2>
